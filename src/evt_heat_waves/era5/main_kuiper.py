...
from evt_heat_waves.era5.cli import (
    parse_args_era5_fit,
    check_kuiper_config_compatability,
)
...
def main():
    ...
    try:
        ...
        raise ValueError(
            f"Runner for with/without MPI doesn't exist for current setup."
        )

    logger.info(
        f"Doing Kuiper statistics analysis for config: {args.grid}|MPI={args.mpi}"
    )
    logger.info("-" * width)
