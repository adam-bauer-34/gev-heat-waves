...
def main():
    ...
    for i, n in enumerate(sample_sizes):
        ...
        est_params = _mle_fit(samples, fit_type='nonstat', SAMPLE_THRES=1)
        se = get_standard_errors(est_params, samples, fit_type='nonstat')

        print(f"Estimated parameters: {est_params}")
        print(f"Standard errors: {se}")
        print(f"L2 distance from true parameters: {compute_l2(est_params, theta_tr)}")
        print("-" * width)
