...
def pproc_amip(logger, args):
    """Preprocess AMIP data to be analyzed in EVT pipeline.
    ...
    data_path_mean = AMIP_PATH / 'tas_annual_mean' / 'raw'
...
def _make_meta_and_qc(logger, args):
    """Make meta data and quality control files for AMIP ensemble.
    ...
    for var in vars:
        ...
        for f in files:
            ...
            # --------------------------------
            # Step 2: Data Quality Controlling 
            # --------------------------------
