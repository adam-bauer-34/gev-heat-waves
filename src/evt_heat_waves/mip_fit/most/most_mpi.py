...
def process_single_fit(logger, args, var, anom_type, model_with_most, mem, fpath, rank):
    ...
    try:
        logger.info(
            f"[Rank {rank}] Working on {var}:{model_with_most}:{mem} - {anom_type} fit"
        )
        ...
        # do fitting
        ds_fit = ds_mle_fit(args, ds, var_name=var_name, fit_dim="year", all_mems=True)
        ...
        gev_param_names = [f"{p}_{sfx}" for p in MLE_FIT_ATTRS[args.fit]["param_names"]]
...
def combine_results_into_datasets(logger, args, all_results, model_with_most, fpath):
    ...
    for (var, anom_type), group_data in grouped.items():
        ...
        # Create dataset
        ds_combined = xr.Dataset(data_vars=data_vars, coords=coords, attrs=attrs_dict)
        ...
        gev_dir = (
            head_data_path / var / "gev"
            if not args.debug
            else head_data_path / var / "gev_debug"
        )
...
def find_model_with_most_members(var, CMIPConfig, data_path, model=None):
    ...
    tied_models = None
    if len(max_inds) > 1:
        all_model_names = np.array([m.name for m in CMIPConfig.iter_active_models(var)])
