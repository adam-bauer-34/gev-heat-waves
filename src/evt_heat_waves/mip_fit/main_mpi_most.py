...
from evt_heat_waves.mip_fit.most.most_mpi import (
    process_single_fit,
    print_summary,
    combine_results_into_datasets,
    find_model_with_most_members,
)
...
def main():
    ...
    if rank == 0:
        ...
        logger.info(
            f"Doing GEV fitting for config: {args.data}|{args.member_config}|MPI=True"
        )
        ...
        for var in vars:
            ...
            # Use the model requested on the CLI, else fall back to the model
            # with the most members in the config data
            model_with_most, fpath, n_members, tied_models = (
                find_model_with_most_members(
                    var, CMIPConfig, data_path, model=args.model
                )
            )

            if args.model is not None:
                message = (
                    f"Using {model_with_most} as requested on the CLI "
                    f"(has {n_members} members)."
                )
            else:
                message = (
                    f"Identified {model_with_most} as model with most ensemble "
            ...
            if tied_models:
                ...
                if "MIROC6" in tied_models:
                    ...
                    logger.info(
                        "Tiebreaker: selected MIROC6 for consistency with CMIP6"
                    )
                else:
                    logger.warning(
                        "No MIROC6 in tied models, proceeding with first choice"
                    )
