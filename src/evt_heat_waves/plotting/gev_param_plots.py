...
# set labels
colorbar_labels = {'loc': r'$\mu$ ($^\circ$C)', 'loc_t': r"$\mu_{trend}$ ($^\circ$C/decade)", 
                   'scale': r'$\sigma$ ($^\circ$C)', 'scale_t': r"$\sigma_{trend}$ ($^\circ$C/decade)",
                   'shape': r'$\xi$ ($-$)', 'shape_t': r"$\xi_{trend}$ (decade$^{-1}$)"}
...
param_num_to_grid = {
    ...
    3: {'grid': (3, 1), 'figsize': (10, 20),
        'subplot_adjust': {'top': 0.6, 'bottom': 0.1}},
    4: {'grid': (2, 2), 'figsize': (12, 12),
        'subplot_adjust': {'left': 0.05, 'right': 0.85, 'top': 0.85, 'bottom': 0.15}},
    5: {'grid': (2, 3), 'figsize': (18, 12),
        'subplot_adjust': {'left': 0.05, 'right': 0.85, 'top': 0.85, 'bottom': 0.15}},
    6: {'grid': (3, 2), 'figsize': (20, 30),
        'subplot_adjust': {'top': 0.6, 'bottom': 0.1}},
}
 
 
def plot_gev_parameters(ds_stat, ds_trend, N_params, anom_type,
                        param_panel_order, ex_type='max',
    ...
    das = [
        ds_trend[f'{param}_{anom_var_name}'] / len(ds_trend.year.values) * 10.
        if param in trend_params
        else ds_stat[f'{param}_{anom_var_name}']
        for param in param_panel_order
    ]
 
    for idx, (ax, da, param) in enumerate(zip(axes.flatten(),
                                              das,
                                              param_panel_order)):
        # Add map features
        ...
        if param in always_positive:
            ...
            norm = mcolors.BoundaryNorm(np.linspace(vmin, vmax, HALF_BINS + 1), ncolors=HALF_BINS)
        ...
        if param in trend_params:
            ...
            se = ds_trend[f'se_{param}_{anom_var_name}'] / n_years * 10.
    ...
    fig.subplots_adjust(
       **param_num_to_grid[N_params]['subplot_adjust']
    )
...
def print_trend_significance(ds, fit, anom_type, n_se=2):
    ...
    for param in MLE_FIT_ATTRS[fit]['param_names']:
        ...
        da = ds[f'{param}_anom_{anom_type}'] / n_years * 10.
        se = ds[f'se_{param}_anom_{anom_type}'] / n_years * 10.
        ...
        print(f"\n{param} (CI: +/- {n_se} SE)")
        print(f"  Confident positive trend:  {pct_sig_pos_of_total:5.1f}% of all land  |  "
              f"{pct_sig_pos_of_pos:5.1f}% of positive-trend cells")
        print(f"  Confident negative trend:  {pct_sig_neg_of_total:5.1f}% of all land  |  "
...
if __name__ == '__main__':
    ...
    plot_gev_parameters(
        ds_stat, ds_nonstat, N_params=3, anom_type=anom_type, param_panel_order=['loc', 'scale', 'loc_t'],
        ex_type=ex_type, save_figs=save_figs,
        fname=f'era5-t2m-{ex_type}-{anom_type}-gev-parameters-{GRID}-{TMIN}-hybrid', output_dir=FIGS_PATH
    )