"""Main file for GEV fitting of AMIP model with most members - MPI parallelized.
...
To run: 
    srun -n <ntasks> python main_amip_most_mems_mpi.py STAT
...
def combine_results_into_datasets(all_results, model_with_most, STAT, data_path_root, width):
    ...
    for (var, fit_type), group_data in grouped.items():
        ...
        gev_dir = data_path_root / 'AMIP' / var / 'gev'
